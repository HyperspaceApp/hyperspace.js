...
import { agent, spaceCashToHastings, call, hastingsToSpaceCash, isRunning, connect, errCouldNotConnect } from '../src/hyperspace.js'
...
const hastingsPerSpaceCash = new BigNumber('1000000000000000000000000')

describe('hyperspace.js wrapper library', () => {
	describe('unit conversion functions', () => {
		it('converts from space cash to hastings correctly', () => {
			const maxSC = new BigNumber('100000000000000000000000')
			for (let i = 0; i < 999; i++) {
				const sc = maxSC.times(Math.trunc(Math.random() * 100000) / 100000)
				const expectedHastings = sc.times(hastingsPerSpaceCash)
				expect(spaceCashToHastings(sc).toString()).to.equal(expectedHastings.toString())
			}
		})
		it('converts from hastings to space cash correctly', () => {
			const maxH = new BigNumber('10').toPower(150)
			for (let i = 0; i < 999; i++) {
				const h = maxH.times(Math.trunc(Math.random() * 100000) / 100000)
				const expectedSpaceCash = h.dividedBy(hastingsPerSpaceCash)
				expect(hastingsToSpaceCash(h).toString()).to.equal(expectedSpaceCash.toString())
			}
		})
		it('does not lose precision during unit conversions', () => {
			// convert from base unit -> space cash n_iter times, comparing the (n_iter-times) converted value at the end.
			// if precision loss were occuring, the original and the converted value would differ.
			const n_iter = 10000
			const originalSpaceCash = new BigNumber('1337338498282837188273')
			let convertedSpaceCash = originalSpaceCash
			for (let i = 0; i < n_iter; i++) {
				convertedSpaceCash = hastingsToSpaceCash(spaceCashToHastings(convertedSpaceCash))
			}
			expect(convertedSpaceCash.toString()).to.equal(originalSpaceCash.toString())
		})
	})
